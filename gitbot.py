...
class TwitterBot:
    ...
    def get_trending_topics(self):
        try:
            response = self.twitter.get("https://api.twitter.com/2/trends/place?id=1")
            if response.status_code == 200:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if tweets.status_code == 200:
                        recent_tweets.extend(tweets.json()['data'])
            except Exception as e:
        ...
        try:
            response = openai.ChatCompletion.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": f"Reply to: {tweet['text']}"}
                ],
                max_tokens=100,
                temperature=0.9
            )
